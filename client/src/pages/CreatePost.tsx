...
const CreatePost = () => {
    ...
    if (form.prompt) {
      try {
        ...
        const response = await fetch(
          "https://dall-e-5d0p.onrender.com/api/v1/dalle",
          {
            method: "POST",
  ...
  const handleSubmit = async (e: ChangeEvent<HTMLFormElement>) => {
    ...
    if (form.prompt && form.photo) {
      ...
      try {
        const response = await fetch(
          "https://dall-e-5d0p.onrender.com/api/v1/post",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
      ...
      <form className="mt-16 max-w-3xl" onSubmit={handleSubmit}>
        ...
        <div className="mt-5 flex gap-5">
          <button
            ...
            className="text-white bg-green-700 fontmedium rounded-md text-sm w-full sm:w-auto px-5 py-2.5 text-center"
          >
